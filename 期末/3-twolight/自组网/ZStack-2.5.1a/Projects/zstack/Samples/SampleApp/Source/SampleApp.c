  ...
  if ( events & SYS_EVENT_MSG )
  {
    ...
    while ( (MSGpkt = (afIncomingMSGPacket_t *)osal_msg_receive( SampleApp_TaskID )) )
    {
      ...
      case AF_INCOMING_MSG_CMD:
        ...
        uint8 debugStr[50];
        if(d[0]>=50||d[1]>=50){P0_7=0;m=1;}
        else if(d[0]<50&&d[0]<50){P0_7=1;m=0;}
        sprintf(debugStr, "LIGHT1:%d LIGHT2:%d, bu:%d", d[0],d[1],m);
  ...
  case SAMPLEAPP_P2P_CLUSTERID: 
    {
      ...
      t[2] = pkt->cmd.Data[6];//pc
      if(t[2]==1)
      {
        d[0]=t[1];
        sprintf(debugStr, "Light1:%d", t[1]);
        HalLcdWriteString((char*)debugStr, HAL_LCD_LINE_3 );
      }
      else
      {
        d[1]=t[1];
        sprintf(debugStr, "Light2:%d", t[1]);
...
void SampleApp_Send_P2P_Message( void )
{
  ...
  sprintf(debugStr, "DO:%d AO:%d", buff, temp[1]);
  HalLcdWriteString((char*)debugStr, HAL_LCD_LINE_3 );
  sprintf(debugStr, "Temp:%d, pc:%d", temp[0],pc);
  HalLcdWriteString((char*)debugStr, HAL_LCD_LINE_4 );
  
  sprintf(debugStr, "DO:%d AO:%d Temp:%d,pc:%d", buff, temp[1],temp[0],pc);
  HalUARTWrite(0, debugStr, osal_strlen(debugStr));
  HalUARTWrite(0, "\r\n", 2);
