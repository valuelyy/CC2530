...
#define AUTOMODE      1
#define UNAUTOMOD     0
uint8 buzzerSwitch = 0;
uint8 buzzerConState = AUTOMODE;
...
{
  uint8 keys = 0;
  //HalLcdWriteString("123", HAL_LCD_LINE_3 );
/* if ((HAL_KEY_JOY_MOVE_PORT & HAL_KEY_JOY_MOVE_BIT))  // Key is active HIGH 
  {
    keys = halGetJoyKeyInput();
  }
*/
  //HalLcdWriteString("1", HAL_LCD_LINE_3);
  if (!P0_1)//S0
  {
    keys |= HAL_KEY_SW_1; 
    //HalLcdWriteString("2", HAL_LCD_LINE_3 );
  }
  if (!P2_0)//S1 
  {
    keys |= HAL_KEY_SW_6; 
    //HalLcdWriteString("3", HAL_LCD_LINE_3 );
  }
  //HalLcdWriteString("1", HAL_LCD_LINE_3 );
  if (!Hal_KeyIntEnable)
  {
