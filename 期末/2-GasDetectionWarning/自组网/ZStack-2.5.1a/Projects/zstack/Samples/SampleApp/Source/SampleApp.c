  ...
  if ( events & SYS_EVENT_MSG )
  {
    ...
    while ( (MSGpkt = (afIncomingMSGPacket_t *)osal_msg_receive( SampleApp_TaskID )) )
    {
      ...
      case AF_INCOMING_MSG_CMD:
        ...
        uint8 debugStr[50];
        sprintf(debugStr, "l:%2d", light);
        HalLcdWriteString((char*)debugStr, HAL_LCD_LINE_3 );
        sprintf(debugStr, "mq2:%2d", gas);
...
void SampleApp_Send_P2P_Message( void )
{
  ...
  if(LIGHTORGAS == LIGHT)
  {
    sprintf(debugStr, "l:%2d ls:%2d", buff, temp[1]);
  }
  if(LIGHTORGAS == GAS)
  {
    sprintf(debugStr, "g:%2d gs:%2d", buff, temp[1]);
  }
  HalLcdWriteString((char*)debugStr, HAL_LCD_LINE_3 ); 
  
  sprintf(debugStr, "temprature:%2d", temp[0]);
  HalLcdWriteString((char*)debugStr, HAL_LCD_LINE_4 );
  if(LIGHTORGAS == LIGHT)
    sprintf(debugStr, "light:%2d lightstrenge:%2d Temp:%2d", buff, temp[1],temp[0]);
  if(LIGHTORGAS == GAS)
    sprintf(debugStr, "mq2:%2d mq2strenge:%2d Gas:%2d", buff, temp[1],temp[0]);
  HalUARTWrite(0, debugStr, osal_strlen(debugStr));
  HalUARTWrite(0, "\r\n", 2);
