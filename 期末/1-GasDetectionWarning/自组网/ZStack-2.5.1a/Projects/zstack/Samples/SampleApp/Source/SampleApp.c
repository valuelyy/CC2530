  ...
  if ( events & SYS_EVENT_MSG )
  {
    ...
    while ( (MSGpkt = (afIncomingMSGPacket_t *)osal_msg_receive( SampleApp_TaskID )) )
    {
      ...
      case AF_INCOMING_MSG_CMD:
        ...
        uint8 debugStr[50];
        sprintf(debugStr, "Light:%2d", light);
        HalLcdWriteString((char*)debugStr, HAL_LCD_LINE_3 );
        sprintf(debugStr, "GAS:%2d", gas);
  ...
  case SAMPLEAPP_P2P_CLUSTERID: 
    {
      ...
      uint8 t[3];
      t[0] = pkt->cmd.Data[4];//温度
      t[1] = pkt->cmd.Data[5];//光照or气体
      t[2] = pkt->cmd.Data[6];//标志位判断传送为气体or光照
      if(buzzerConState)
      {
        if(t[1]>=THRESHOLD && t[2] == GAS){P0_7=0;}
        else if(t[1]<THRESHOLD && t[2] == GAS){P0_7=1;}
      }
      ...
      //uint8 debugStr[50];
      //sprintf(debugStr, "Light:%3d", t[1]);
      //HalLcdWriteString((char*)debugStr, HAL_LCD_LINE_3 );
      //sprintf(debugStr, "Temp:%3d", t[0]);
      //HalLcdWriteString((char*)debugStr, HAL_LCD_LINE_4 );
      
      sendGtwReport(nwkAddr, parentShortAddr, t[0],0);
    }
...
void SampleApp_Send_P2P_Message( void )
{
  ...
  if(LIGHTORGAS == GAS)
  {
    sprintf(debugStr, "GAS:%2d Str:%2d", buff, temp[1]);
  }
  HalLcdWriteString((char*)debugStr, HAL_LCD_LINE_3 ); 
  
  sprintf(debugStr, "Temp:%2d", temp[0]);
  HalLcdWriteString((char*)debugStr, HAL_LCD_LINE_4 );
  if(LIGHTORGAS == LIGHT)
    sprintf(debugStr, "DO:%2d AO:%2d Temp:%2d", buff, temp[1],temp[0]);
  if(LIGHTORGAS == GAS)
    sprintf(debugStr, "DO:%2d AO:%2d Gas:%2d", buff, temp[1],temp[0]);
