 ...
 * LOCAL FUNCTIONS
 */
static void TestApp_ProcessZDOMsgs( zdoIncomingMsg_t *inMsg );
static void TestApp_HandleKeys( byte shift, byte keys );
static void TestApp_MessageMSGCB( afIncomingMSGPacket_t *pckt );
static void TestApp_SendTheMessage( void );

#if defined( IAR_ARMCM3_LM )
static void TestApp_ProcessRtosMessage( void );
#endif
...
void TestApp_Init( uint8 task_id )
{
  ...
  RTOS_RegisterApp( task_id, TestApp_RTOS_MSG_EVT );
#endif
}
...
uint16 TestApp_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  if ( events & TestApp_RTOS_MSG_EVT )
  {
    // Process message from RTOS queue
    TestApp_ProcessRtosMessage();

    // return unprocessed events
    return (events ^ TestApp_RTOS_MSG_EVT);
  }
#endif
 ...
 * @return  none
 */
static void TestApp_HandleKeys( uint8 shift, uint8 keys )
{
  zAddrType_t dstAddr;
 ...
 * @return  none
 */
static void TestApp_MessageMSGCB( afIncomingMSGPacket_t *pkt )
{
